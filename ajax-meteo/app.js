...
async function main(){
    ...
    // 2. choper la ville frace a l'adresse IP: http//freegeoip.net/json/adresseIPDuMec
    const ville = await fetch('http://freegeoip.net/json/' + ip)
        .then(resultat => resultat.json())
        .then(json => json.city)


    const meteo = await fetch('http://api.openweathermap.org/data/2.5/weather?q=$%7Bville%7D&appid=8e602b9ea28ed4f9f8fc97a5f6d1105c&lang=fr&units=metric')
